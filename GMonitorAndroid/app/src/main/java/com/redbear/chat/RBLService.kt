...
class RBLService : Service() {
     ...
     */
    val supportedGattService: BluetoothGattService?
        get() = if (mBluetoothGatt == null) null else mBluetoothGatt!!.getService(UUID_BLE_SHIELD_SERVICE)
    ...
    private fun broadcastUpdate(
        ...
        if (UUID_BLE_SHIELD_RX == characteristic.uuid) {
            val rx = characteristic.value
    ...
    inner class LocalBinder : Binder() {
        internal val service: RBLService
            get() = this@RBLService
    }
    ...
    /**
     * Request a read on a given `BluetoothGattCharacteristic`. The read
    ...
    companion object {
        private val TAG = RBLService::class.java.simpleName
